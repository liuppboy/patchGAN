...
class Inpainting(object):
    def __init__(self, sess, batch_size=32, iter_steps=50000, learning_rate=1e-3, 
                 sample_size=100, beta1=0.5, is_load_model=True, dataset_name='celeba', 
        ...
        with tf.variable_scope('generator'):
            with tf.variable_scope('encoder', reuse=reuse) as scope:
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, kernel_size=3, stride=1, padding='SAME', 
                    ...
                    net = {}
                    net['conv1'] = slim.conv2d(x, 32, kernel_size=5, scope='conv1')
                    net['conv2'] = slim.conv2d(net['conv1'], 64, stride=2, scope='conv2')
                    net['conv3'] = slim.conv2d(net['conv2'], 64, scope='conv3')
                    net['conv4'] = slim.conv2d(net['conv3'], 128, stride=2, scope='conv4')
                    
                    net['conv5'] = slim.conv2d(net['conv4'], 128, scope='conv5')
                    net['conv6'] = slim.conv2d(net['conv5'], 128, scope='conv6')
                    
                    net['conv7'] = slim.conv2d(net['conv6'], 128, rate=2, scope='conv7')
                    net['conv8'] = slim.conv2d(net['conv7'], 128, rate=4, scope='conv8')
                    net['conv9'] = slim.conv2d(net['conv8'], 128, rate=8, scope='conv9')
                    
                    net['conv10'] = slim.conv2d(net['conv9'], 128, scope='conv10')
                    net['conv11'] = slim.conv2d(net['conv10'], 128, scope='conv11')
                    
                        
                with slim.arg_scope([slim.conv2d_transpose], activation_fn=tf.nn.relu, kernel_size=4, stride=2, padding='SAME', 
                                    normalizer_fn=batch_norm, normalizer_params={'train': train}):
                    
                    net['deconv1'] = slim.conv2d_transpose(net['conv11'], 64, scope='deconv1')
                    net['conv12'] = slim.conv2d(net['deconv1'], 64, 3, normalizer_fn=batch_norm, 
                                                normalizer_params={'train': train}, scope='conv12')
                    net['deconv2'] = slim.conv2d_transpose(net['conv12'], 32, scope='deconv2')
                    net['conv13'] = slim.conv2d(net['deconv2'], 16, 3, normalizer_fn=batch_norm, 
    ...
    def discriminator_wgan_gp(self, global_x, local_x, train, reuse=None):
        with tf.variable_scope('discriminator', reuse=reuse) as scope: 
            with slim.arg_scope([slim.conv2d], kernel_size=5, stride=2, activation_fn=lrelu):
                net['g_conv1'] = slim.conv2d(global_x, 32, scope='g_conv1')
                net['g_conv2'] = slim.conv2d(net['g_conv1'], 64, scope='g_conv2')
                net['g_conv3'] = slim.conv2d(net['g_conv2'], 128, scope='g_conv3')
                net['g_flatten'] = slim.flatten(net['g_conv3']) 
                
                net['l_conv1'] = slim.conv2d(local_x, 32, scope='l_conv1')
                net['l_conv2'] = slim.conv2d(net['l_conv1'], 64, scope='l_conv2')
                net['l_conv3'] = slim.conv2d(net['l_conv2'], 128, scope='l_conv3')
                net['l_flatten'] = slim.flatten(net['l_conv3'])
                
                net['flatten'] = tf.concat([net['g_flatten'], net['l_flatten']], axis=-1)
                
                net['fc'] = slim.fully_connected(net['flatten'], 1, activation_fn=None, scope='fc')
    ...
    def train_discriminator_wgan_gp(self):
        tfrecord_name = ''.join(['../datasets/', self.dataset_name, '/train.tfrecords'])
        filename_queue  = tf.train.string_input_producer([tfrecord_name], capacity=32, shuffle=True)
        self.batch_img, self.batch_mask, self.batch_img_mask, self.batch_local_patch_coord = generate_batch(
            filename_queue, min_queue_examples=2000, batch_size=self.batch_size, num_threads=16) 
        self.bulid_model(train=False)
        self.lr_decay = tf.train.polynomial_decay(
            self.learning_rate, self.global_step, decay_steps=4e4, end_learning_rate=1e-5)
    ...
    def train_feed(self):
        test_img_list = glob.glob('../datasets/celeba/train/img/*.jpg')
        test_mask_list = glob.glob('../datasets/celeba/train/mask/*.jpg')
        save_img_num = 32
        sample_img_dir = []
        for i in range(save_img_num):
            sample_img_dir.append('/'.join(['sample/train', test_img_list[i].split('/')[-1][:-4]]))
        ...
        self.bulid_model(train=True)
        self.lr_decay = tf.train.polynomial_decay(self.learning_rate, self.global_step, decay_steps=1e4, end_learning_rate=1e-5)
        ...
        summary_writer = tf.summary.FileWriter(logdir='summary')
        steps = np.arange(20000, 21000+1, 200, dtype='int32')
        self.sess.run(tf.global_variables_initializer())
        for step in steps:
            ...
            #for i in range(1, 2001):
                #_, weighted_mse_loss = self.sess.run([self.g_optim, self.weighted_mse_loss], 
                                                #feed_dict={self.batch_img_mask: img_mask, 
                                                           #self.batch_img: img, 
                                                           #self.batch_mask: mask})
                #if i % 10 == 0:
                    #print('step %d: weighted_mse_loss %.6f' % (i, weighted_mse_loss))
                
                #if i % 200 == 0:
                    #self.saver.save(self.sess, '/'.join([self.checkpoint_dir, self.model_name]), global_step=step+i)
            ...
            for j in range(batch_size):
                img_name = '/'.join([sample_img_dir[j], '%06d.jpg' % int(step+i)])
                misc.imsave(img_name, sample_result[j])                    
    ...
    def test(self):
        ...
        summary_writer = tf.summary.FileWriter(logdir='summary')
        steps = np.arange(50000, 100000+1, 2000, dtype='int32')
        self.sess.run(tf.global_variables_initializer())
        for step in steps:
            model_name = 'checkpoints/dcgan_model-%d' % step
